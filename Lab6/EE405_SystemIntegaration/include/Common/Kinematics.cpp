...
Eigen::MatrixXd Kinematics::GetJacobianMatrix(const Eigen::VectorXd &current_q)
{
    ...
    Eigen::Vector3d r3e_w=Rw3*r3e_3; // A vector pointing the end-effector frame from the ID 3 motor frame represented in the world frame
    Eigen::Vector3d r2e_w=r3e_w+Rw2*(r23_2); // A vector pointing the end-effector frame from the ID 2 motor frame represented in the world frame
    Eigen::Vector3d r1e_w=r2e_w+Rw1*(r12_1); // A vector pointing the end-effector frame from the ID 1 motor frame represented in the world frame
    Eigen::Vector3d r0e_w=r1e_w+Rw0*(r01_0); // A vector pointing the end-effector frame from the ID 0 motor frame represented in the world frame

    Eigen::MatrixXd JacobianTranslationEE(3, 4); JacobianTranslationEE.setZero();
    // JacobianTranslationEE=//Implement here
    JacobianTranslationEE << Pw0.cross(r0e_w), Pw1.cross(r1e_w), Pw2.cross(r2e_w), Pw3.cross(r3e_w);
