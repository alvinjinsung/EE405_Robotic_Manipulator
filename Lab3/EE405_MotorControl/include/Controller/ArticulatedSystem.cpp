...
{   
    ...
    for (size_t i = 0; i < dof; i++)
    {
        ...
        // qpos(i)=NormalizeAngle(qpos(i), minimum_joint_angle, supremum_joint_angle);
    }
    return qpos;
}
...
void ArticulatedSystem::SetGoalPosition(const Eigen::VectorXd &target_position)
{
    for (int i = 0; i < dof; i++)
    {
        ...
        // double goal_position_count =  (NormalizeAngle(target_position(i), minimum_joint_angle, supremum_joint_angle) / (2*M_PI)) * position_resolution.at(i);
